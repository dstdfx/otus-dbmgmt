...
SET @PROVIDER_ID_A := UUID_TO_BIN(UUID()),
    ...
    @PROVIDER_ID_D := UUID_TO_BIN(UUID());
...
VALUES (@PROVIDER_ID_A, 'Provider A', 'example_a@gmail.com', '+123123', @ADDRESS_ID_A),
       ...
       (@PROVIDER_ID_D, 'Provider D', 'example_d@gmail.com', '+123123', @ADDRESS_ID_C);
...
SET @SELL_PRICE_ID_A := UUID_TO_BIN(UUID()),
    @SELL_PRICE_ID_B := UUID_TO_BIN(UUID()),
    @SELL_PRICE_ID_C := UUID_TO_BIN(UUID()),
    @PROVIDER_PRICE_ID_A := UUID_TO_BIN(UUID()),
    @PROVIDER_PRICE_ID_B := UUID_TO_BIN(UUID()),
    @PROVIDER_PRICE_ID_C := UUID_TO_BIN(UUID()),
    @PROVIDER_PRICE_ID_D := UUID_TO_BIN(UUID()),
    @PROVIDER_PRICE_ID_E := UUID_TO_BIN(UUID()),
    @PROVIDER_PRICE_ID_F := UUID_TO_BIN(UUID()),
    @PROVIDER_PRICE_ID_G := UUID_TO_BIN(UUID());

INSERT INTO prices (id, amount, currency_id)
VALUES (@SELL_PRICE_ID_A, 3499.000, @CURRENCY_ID),
       (@SELL_PRICE_ID_B, 6599.000, @CURRENCY_ID),
       (@SELL_PRICE_ID_C, 1799.000, @CURRENCY_ID),
       (@PROVIDER_PRICE_ID_A, 2490.800, @CURRENCY_ID),
       (@PROVIDER_PRICE_ID_B, 5499.000, @CURRENCY_ID),
       (@PROVIDER_PRICE_ID_C, 7499.000, @CURRENCY_ID),
       (@PROVIDER_PRICE_ID_D, 5999.000, @CURRENCY_ID),
       (@PROVIDER_PRICE_ID_E, 6999.000, @CURRENCY_ID),
       (@PROVIDER_PRICE_ID_F, 350.000, @CURRENCY_ID),
       (@PROVIDER_PRICE_ID_G, 375.000, @CURRENCY_ID);


SET @PRODUCT_ID_A_1 := UUID_TO_BIN(UUID()),
    @PRODUCT_ID_A_2 := UUID_TO_BIN(UUID()),
    @PRODUCT_ID_A_3 := UUID_TO_BIN(UUID()),

    @PRODUCT_ID_B_1 := UUID_TO_BIN(UUID()),
    @PRODUCT_ID_B_2 := UUID_TO_BIN(UUID()),
    @PRODUCT_ID_B_3 := UUID_TO_BIN(UUID()),

    @PRODUCT_ID_C_1 := UUID_TO_BIN(UUID()),
    @PRODUCT_ID_C_2 := UUID_TO_BIN(UUID()),
    @PRODUCT_ID_C_3 := UUID_TO_BIN(UUID());

INSERT INTO products (id, name, category_id, brand_id, attributes)
VALUES -- A category products
       (@PRODUCT_ID_A_1, 'Product A_1', @CATEGORY_ID_A, @BRAND_ID_A,
        '{"screen": "16 inch", "resolution": "3072 x 1920 pixels", "ports": {"thunderbolt": 2, "usb": 0}}'),
       (@PRODUCT_ID_A_2, 'Product A_2', @CATEGORY_ID_A, @BRAND_ID_A,
        '{"screen": "16 inch", "resolution": "3072 x 1920 pixels", "ports": {"thunderbolt": 2, "usb": 0}}'),
       (@PRODUCT_ID_A_3, 'Product A_3', @CATEGORY_ID_A, @BRAND_ID_A,
        '{"screen": "16 inch", "resolution": "3072 x 1920 pixels", "ports": {"thunderbolt": 2, "usb": 0}}'),
       -- B category products
       (@PRODUCT_ID_B_1, 'Product B_1', @CATEGORY_ID_B, @BRAND_ID_B,
        '{"screen": "16 inch", "resolution": "3072 x 1920 pixels", "ports": {"thunderbolt": 2, "usb": 0}}'),
       (@PRODUCT_ID_B_2, 'Product B_2', @CATEGORY_ID_B, @BRAND_ID_B,
        '{"screen": "16 inch", "resolution": "3072 x 1920 pixels", "ports": {"thunderbolt": 2, "usb": 0}}'),
       (@PRODUCT_ID_B_3, 'Product B_3', @CATEGORY_ID_B, @BRAND_ID_B,
        '{"screen": "16 inch", "resolution": "3072 x 1920 pixels", "ports": {"thunderbolt": 2, "usb": 0}}'),
       -- C category products
       (@PRODUCT_ID_C_1, 'Product C_1', @CATEGORY_ID_C, @BRAND_ID_C,
         '{"screen": "12 inch", "resolution": "1440 x 820 pixels", "ports": {"thunderbolt": 0, "usb": 3}}'),
       (@PRODUCT_ID_C_2, 'Product C_2', @CATEGORY_ID_C, @BRAND_ID_C,
        '{"screen": "12 inch", "resolution": "1440 x 820 pixels", "ports": {"thunderbolt": 0, "usb": 3}}'),
       (@PRODUCT_ID_C_3, 'Product C_3', @CATEGORY_ID_C, @BRAND_ID_C,
        '{"screen": "12 inch", "resolution": "1440 x 820 pixels", "ports": {"thunderbolt": 0, "usb": 3}}');

INSERT INTO provider_products (id, product_id, provider_id, price_id)
VALUES -- Provider A
       (UUID_TO_BIN(UUID()), @PRODUCT_ID_A_1, @PROVIDER_ID_A, @PROVIDER_PRICE_ID_A),
       (UUID_TO_BIN(UUID()), @PRODUCT_ID_B_1, @PROVIDER_ID_A, @PROVIDER_PRICE_ID_B),
       (UUID_TO_BIN(UUID()), @PRODUCT_ID_C_1, @PROVIDER_ID_A, @PROVIDER_PRICE_ID_C),
       (UUID_TO_BIN(UUID()), @PRODUCT_ID_A_2, @PROVIDER_ID_A, @PROVIDER_PRICE_ID_A),
       (UUID_TO_BIN(UUID()), @PRODUCT_ID_B_2, @PROVIDER_ID_A, @PROVIDER_PRICE_ID_B),
       (UUID_TO_BIN(UUID()), @PRODUCT_ID_C_2, @PROVIDER_ID_A, @PROVIDER_PRICE_ID_C),
       -- Provider B
       (UUID_TO_BIN(UUID()), @PRODUCT_ID_A_1, @PROVIDER_ID_B, @PROVIDER_PRICE_ID_D),
       (UUID_TO_BIN(UUID()), @PRODUCT_ID_B_1, @PROVIDER_ID_B, @PROVIDER_PRICE_ID_D),
       (UUID_TO_BIN(UUID()), @PRODUCT_ID_C_1, @PROVIDER_ID_B, @PROVIDER_PRICE_ID_D),
       (UUID_TO_BIN(UUID()), @PRODUCT_ID_A_2, @PROVIDER_ID_B, @PROVIDER_PRICE_ID_E),
       (UUID_TO_BIN(UUID()), @PRODUCT_ID_B_2, @PROVIDER_ID_B, @PROVIDER_PRICE_ID_E),
       (UUID_TO_BIN(UUID()), @PRODUCT_ID_C_2, @PROVIDER_ID_B, @PROVIDER_PRICE_ID_E),
       (UUID_TO_BIN(UUID()), @PRODUCT_ID_A_3, @PROVIDER_ID_B, @PROVIDER_PRICE_ID_E),
       (UUID_TO_BIN(UUID()), @PRODUCT_ID_B_3, @PROVIDER_ID_B, @PROVIDER_PRICE_ID_E),
       (UUID_TO_BIN(UUID()), @PRODUCT_ID_C_3, @PROVIDER_ID_B, @PROVIDER_PRICE_ID_E),
       -- Provider C
       (UUID_TO_BIN(UUID()), @PRODUCT_ID_A_2, @PROVIDER_ID_C, @PROVIDER_PRICE_ID_C),
       (UUID_TO_BIN(UUID()), @PRODUCT_ID_B_2, @PROVIDER_ID_C, @PROVIDER_PRICE_ID_C),
       (UUID_TO_BIN(UUID()), @PRODUCT_ID_C_2, @PROVIDER_ID_C, @PROVIDER_PRICE_ID_B),
       (UUID_TO_BIN(UUID()), @PRODUCT_ID_A_3, @PROVIDER_ID_C, @PROVIDER_PRICE_ID_C),
       (UUID_TO_BIN(UUID()), @PRODUCT_ID_B_3, @PROVIDER_ID_C, @PROVIDER_PRICE_ID_C),
       (UUID_TO_BIN(UUID()), @PRODUCT_ID_C_3, @PROVIDER_ID_C, @PROVIDER_PRICE_ID_B);

INSERT INTO orders (id, customer_id, product_id, price_id, address_id, quantity, status)
VALUES (UUID_TO_BIN(UUID()), @CUSTOMER_ID_A, @PRODUCT_ID_A_1, @SELL_PRICE_ID_A, @ADDRESS_ID_A, 5, 'COMPLETED'),
       (UUID_TO_BIN(UUID()), @CUSTOMER_ID_A, @PRODUCT_ID_B_1, @SELL_PRICE_ID_B, @ADDRESS_ID_A, 3, 'COMPLETED'),
       (UUID_TO_BIN(UUID()), @CUSTOMER_ID_A, @PRODUCT_ID_C_1, @SELL_PRICE_ID_C, @ADDRESS_ID_A, 2, 'COMPLETED'),
       (UUID_TO_BIN(UUID()), @CUSTOMER_ID_B, @PRODUCT_ID_C_2, @SELL_PRICE_ID_C, @ADDRESS_ID_B, 2, 'PROCESSING'),
       (UUID_TO_BIN(UUID()), @CUSTOMER_ID_C, @PRODUCT_ID_B_2, @SELL_PRICE_ID_B, @ADDRESS_ID_C, 1, 'COMPLETED'),
       (UUID_TO_BIN(UUID()), @CUSTOMER_ID_A, @PRODUCT_ID_A_2, @SELL_PRICE_ID_A, @ADDRESS_ID_A, 5, 'ON_HOLD'),
       (UUID_TO_BIN(UUID()), @CUSTOMER_ID_A, @PRODUCT_ID_B_3, @SELL_PRICE_ID_B, @ADDRESS_ID_A, 3, 'ON_HOLD'),
       (UUID_TO_BIN(UUID()), @CUSTOMER_ID_A, @PRODUCT_ID_B_3, @SELL_PRICE_ID_B, @ADDRESS_ID_A, 9, 'PROCESSING');
