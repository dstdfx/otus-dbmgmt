...
VALUES (@SELL_PRICE_ID_A, 3.200, @CURRENCY_ID),
       (@SELL_PRICE_ID_B, 6.700, @CURRENCY_ID),
       (@SELL_PRICE_ID_C, 17.900, @CURRENCY_ID),
       (@PROVIDER_PRICE_ID_A, 2.800, @CURRENCY_ID),
       (@PROVIDER_PRICE_ID_B, 5.000, @CURRENCY_ID),
       (@PROVIDER_PRICE_ID_C, 15.000, @CURRENCY_ID);
...
VALUES (UUID_TO_BIN(UUID()), @PRODUCT_ID_A, @PROVIDER_ID_A, @PROVIDER_PRICE_ID_A),
       (UUID_TO_BIN(UUID()), @PRODUCT_ID_B, @PROVIDER_ID_B, @PROVIDER_PRICE_ID_B),
       (UUID_TO_BIN(UUID()), @PRODUCT_ID_C, @PROVIDER_ID_C, @PROVIDER_PRICE_ID_C);

INSERT INTO orders (id, customer_id, product_id, price_id, address_id, quantity, status)
VALUES (UUID_TO_BIN(UUID()), @CUSTOMER_ID_A, @PRODUCT_ID_A, @SELL_PRICE_ID_A, @ADDRESS_ID_A, 5, 'COMPLETED'),
       (UUID_TO_BIN(UUID()), @CUSTOMER_ID_A, @PRODUCT_ID_B, @SELL_PRICE_ID_B, @ADDRESS_ID_A, 3, 'COMPLETED'),
       (UUID_TO_BIN(UUID()), @CUSTOMER_ID_A, @PRODUCT_ID_C, @SELL_PRICE_ID_C, @ADDRESS_ID_A, 2, 'COMPLETED'),
       (UUID_TO_BIN(UUID()), @CUSTOMER_ID_B, @PRODUCT_ID_C, @SELL_PRICE_ID_C, @ADDRESS_ID_B, 2, 'PROCESSING'),
       (UUID_TO_BIN(UUID()), @CUSTOMER_ID_C, @PRODUCT_ID_B, @SELL_PRICE_ID_B, @ADDRESS_ID_C, 1, 'COMPLETED'),
       (UUID_TO_BIN(UUID()), @CUSTOMER_ID_A, @PRODUCT_ID_A, @SELL_PRICE_ID_A, @ADDRESS_ID_A, 5, 'ON_HOLD'),
       (UUID_TO_BIN(UUID()), @CUSTOMER_ID_A, @PRODUCT_ID_B, @SELL_PRICE_ID_B, @ADDRESS_ID_A, 3, 'ON_HOLD'),
       (UUID_TO_BIN(UUID()), @CUSTOMER_ID_A, @PRODUCT_ID_B, @SELL_PRICE_ID_B, @ADDRESS_ID_A, 9, 'PROCESSING');
